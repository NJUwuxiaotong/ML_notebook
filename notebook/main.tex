...
\title{Notebook of The Stochastic Gradient Descent Family of Algorithms} % Title of the book

\author[Xiaotong Wu]{Xiaotong Wu} % Author
...
%\thispagestyle{empty}
%\openepigraph{The public.}{Paul Rand, {\itshape Design, Form, and Chaos}}
%\vfill
%\openepigraph{A designer knows that.}{Antoine de Saint-Exup\'{e}ry}
%\vfill
%\openepigraph{\ldots the designer of}{Donald E. Knuth}
...
\newthought{Online Learning} techniques are used for estimating the unknown parameter vector, which are time-iterative schemes to update the available estimate every time a measurement set is acquired.

A class of online/adaptive algorithms that springs from the classical gradient descent method for optimization.
...
\label{ch:steepest_descent}

\newthought{Gradient Descent} for iterative minimizatio of a differentiable cost function, \ie, $\min \bm{ J(\theta) }$, $ \bm{\theta}\in R^l $.
\begin{equation}
\begin{aligned}
\textrm{initial:} & \quad \bm{\theta}^{(0)} \\ 
\textrm{iterative:} & \quad \bm{\theta}^{(i)} = \bm{\theta}^{(i-1)} + \mu_i                    \Delta\bm{\theta}^{(i)},\quad i>0, \mu_i>0
\end{aligned}
\end{equation}

The important differences of various approaches are in the way that $ \mu_i $ and $ \Delta\bm{\theta}^{(i)} $ are chosen.
\begin{itemize}
\item $ \mu_i\Delta\bm{\theta}^{(i)} $: the \textit{update direction} or \textit{the search direction};
\item $ \mu_i $: the step size or the step length (be constant or change).
\end{itemize}
\textbf{Gradient descent}: chosing $ \Delta\bm{\theta}^{(i)} \rightarrow J(\bm{\theta}^(i)) < J(\bm{\theta}^(i-1)) $, except at a minimizer, $ \bm{\theta}_* $. 
...
\bigskip
The design of the front matter in T (\eg)

\newthought{The full title page} of each of the books varies.
...
\newthought{The tables of contents} in Tufte's books give us our f
